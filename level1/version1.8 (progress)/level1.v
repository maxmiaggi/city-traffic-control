...
// version 1.8:   Replacing 't' logic with a counter logic (In Progress...)

/************************
  
  LUT
  
  ROUND		T[3]	T[2]	T[1]	T[0]
  0			0		15		15		15
  1			15		1		2		3
  2			15		4		5		6
  3			15		7		8		9
...
module level1(T, T1, T2, T3, T4, S1, S2, S3, S4, clk, clear, delay_counter);
  ...
  // introduced in v1.8
  // replacement of 't' logic
  reg	  round[1:0];				// to count the number of rounds
  reg	  [3:0] LUT [0:3][0:3];		// LUT to store values (max 15)
  reg	  [4:0] TVAL;				// 'TVAL' is replacement of 'T' (v1.8) (max 30)
  reg	  [4:0] TTVAL;				// TTVAL is temp variable for TVAL for task
  ...
  *****/
  ...
  task calc_prob_next_state; // probable next state as output
    ...
    input  [4:0] tval;             // cycle variable as input
    output [4:0] ttval;			   // modified cycle variable as output
    begin
  ...
  begin
	  if (clear) begin
		...
		// initialize LUT when 'clear' is pressed
		LUT[0][0] = 4'd0;
		LUT[0][1] = 4'd15;
		LUT[0][2] = 4'd15;
		LUT[0][3] = 4'd15;
      ...
      case (state)
        ...
        `T1Y: begin
                ...
                if(Y2Rsignal) begin									 // wait from Y to R (v1.4)
                  calc_prob_next_state(next_state, S1, S2, S3, S4, TVAL, TTVAL); // calling task to update next_state and T
                  // delay_counter = 4'd0;                              // clear counter upon state change (v1.2)
                  delay_counter_clear = 1'b1;						 // this signal will clear the counter (v1.4)
                  TVAL = TTVAL;											 // Assign TTVAL to TVAL
                end
                else begin
				  delay_counter_clear = 1'b0;						 // this is to ensure counter keeps ticking
				end
